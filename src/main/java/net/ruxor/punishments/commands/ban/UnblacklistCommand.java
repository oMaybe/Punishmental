...
public class UnblacklistCommand extends ConsoleCommand {

    public UnblacklistCommand() {
        super("unblacklist", "punish.unblacklist");
        setUsage("Usage: /unblacklist <player> [reason] [-s]");
    }
    ...
    protected void execute(CommandSender sender, String[] args) {
        ...
        if (!targetProfile.isBlacklisted()){
        ...
        Punishment punishment = targetProfile.getActiveBlacklist();
        if (punishment == null) {
        ...
        if (silent){
            for (Player staff : Bukkit.getOnlinePlayers()) {
                if (staff.hasPermission((String) FileUtils.getOrDefaultConfig("punish.punish_silent"))) {
                    staff.sendMessage(Messages.SILENT_UNBLACKLIST_MESSAGE
            ...
            Bukkit.broadcastMessage(Messages.PUBLIC_UNBLACKLIST_MESSAGE
                    .replace("%player%", targetProfile.getPlayerName())
                    .replace("%staff%", sender.getName())
                    .replace("%reason%", reason));
        }
        ...
        String message = CC.Gray + "You have unblacklisted " + CC.Dark_Red + playerName + CC.Gray + " for " + CC.Dark_Red + reason + ".";
